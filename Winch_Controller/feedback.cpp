...
void readEncoder(struct encoder_data * feedback_data){
   ...
   feedback_data->speed_rps = ((current_pos - prev_pos)*1000000/((feedback_data->states_per_turn)*time_step_uS));
   feedback_data->speed_mps = (feedback_data->speed_rps) * 2 * PI * (feedback_data->drum_radius_m);
   feedback_data->position_m += (feedback_data->speed_mps)*time_step_uS/1000000;
